...
My Master's thesis under Professor Abhishek Halder is focused on designing an **Optimal Covariance Steering Algorithm in Continuous Time with Hilbert-Schmidt Terminal Cost for Linear Stochastic Systems over a finite time horizon**. Prior works on the continuous-time Optimal Covariance Steering problem addressed formulations with a hard constraint on the final marginal distribution, effectively finding the most likely stochastic evolution between two probability distributions at two distinct points in time. Our broader motivation is to move toward deriving a custom algorithm for solving boundary value problems with a Wasserstein terminal cost, inspired by foundational ideas in the earlier literature. This work represents a key step in that direction. Building upon this foundation, my current research introduces a soft constraint via the Hilbert–Schmidt Terminal Cost, together with a quadratic cost function for control input and state. This formulation enhances system performance and computational efficiency by optimizing control efforts and the desired system states. For this problem, the necessary conditions of optimality become a coupled matrix ODE two-point boundary value problem. To solve this system of equations, I've designed a **Matricial Recursive Algorithm** that is guaranteed to converge. The proposed algorithm and its analysis make use of the linear fractional transforms parameterized by the state transition matrix of the associated Hamiltonian matrix. The proposed algorithm was tested on a close-proximity rendezvous scenario by modeling the relative motion of a service spacecraft to a target satellite in LEO using Clohessy–Wiltshire dynamics with stochastic disturbances. The code for this project can be found [here](https://github.com/tusharsial/Covariance-Steering). 
